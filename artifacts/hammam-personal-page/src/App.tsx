      ...
      <main>
        ...
        <section id="work" className="mx-auto max-w-[1440px] scroll-mt-20 px-5 py-24 sm:px-8 lg:px-12 lg:py-36" aria-labelledby="work-title">
          ...
          <div className="grid gap-5 lg:grid-cols-[1.4fr_.85fr]">
            <a href="https://norv.ai" target="_blank" rel="noreferrer" className="reveal group relative min-h-[390px] overflow-hidden bg-foreground p-6 text-background sm:min-h-[500px] sm:p-10" data-testid="link-project-norv">
              <div className="absolute inset-0 opacity-30" style={{ backgroundImage: 'radial-gradient(circle at 80% 15%, hsl(var(--accent) / .7) 0 1px, transparent 1px), radial-gradient(circle at 20% 80%, hsl(var(--background) / .35) 0 1px, transparent 1px)', backgroundSize: '28px 28px, 44px 44px' }} />
            ...
            <a href="https://it-bridge.com" target="_blank" rel="noreferrer" className="reveal reveal-delay-1 group flex min-h-[390px] flex-col justify-between border border-border bg-card p-6 sm:min-h-[500px] sm:p-8" data-testid="link-project-it-bridge">
              <div className="flex items-start justify-between"><span className="border border-border px-3 py-1.5 font-mono-custom text-[10px] uppercase tracking-[.15em] text-muted-foreground">Selected / 02</span><ExternalLink size={20} className="text-muted-foreground transition-colors group-hover:text-foreground" /></div>
              <div><div className="mb-7 h-24 w-24 border border-foreground p-3"><div className="flex h-full items-end justify-between gap-1">{[35, 60, 45, 82, 68].map((height, i) => <span className="w-full bg-accent" style={{ height: `${height}%` }} key={i} />)}</div></div><p className="font-mono-custom text-[10px] uppercase tracking-[.15em] text-muted-foreground">IT-Bridge</p><h3 className="mt-2 font-display text-4xl tracking-[-.06em] sm:text-5xl">Connect the gap.</h3><p className="mt-4 max-w-sm text-sm leading-6 text-muted-foreground">{lang === 'en' ? 'A digital bridge between people, technology, and the opportunities in between.' : 'جسر رقمي بين الناس والتقنية والفرص التي تجمعهما.'}</p></div>
            </a>
          </div>
        </section>
